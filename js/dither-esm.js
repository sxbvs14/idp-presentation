/**
 * Dither Hero — React Bits port
 * Uses import maps (esm.sh) — zero bundling.
 */
...
/* ── Shaders (exact from React Bits docs) ── */
...
float cnoise(vec2 P){
  ...
  vec2 fade_xy=fade(Pf.xy), n_x=mix(vec2(n00,n01),vec2(n10,n11),fade_xy.x);
...
function RetroEffectComp({ colorNum, pixelSize, forwardRef }) {
  return React.createElement(WrappedRetro, { ref: forwardRef, colorNum, pixelSize });
}
...
function DitheredWaves({
  ...
  const waveUniformsRef = React.useRef({
  ...
  const prevColor = React.useRef([...waveColor]);
  useFrame(({ clock }) => {
  ...
  return React.createElement(React.Fragment, null,
    React.createElement('mesh', { ref: mesh, scale: [viewport.width, viewport.height, 1] },
    ...
    React.createElement(EffectComposer, null,
      React.createElement(RetroEffectComp, { colorNum, pixelSize })
    ),
